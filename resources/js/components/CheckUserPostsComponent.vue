<template>
  ...
  <div v-for="(post, index) in posts.posts" :key="post" class="block post">
    ...
    <div class="header_post">
      <div>
        <router-link v-if="this.$store.state.user.id != post.id_user.id" :to="{ path: '/user/' + post.id_user.id }"><img
            :src="post.id_user.avatar" alt="" /></router-link>
    ...
    <div class="body_post">
      ...
      <img v-else-if="post.photos.length == 1" :src="'/' + post.photos[0].photo" alt="" />
    </div>
  ...
  methods: {
    ...
    allPosts() {
      ...
      axios
        ...
        .then((res) => {
          this.posts = res.data;
          axios.get(`/api/all/repost/${this.id_user}`, {
            headers: {
              Authorization: `Bearer ${localStorage.getItem("token")}`,
            }
          })
            .then(res => {
              ...
              if (this.posts.posts.length == 0) {
                ...
                this.posts.posts.forEach((post, index) => {
                  ...
                  if (post.id_user.id != this.id_user) {
    ...
    nextPosts() {
      axios
        .get(
          `/api/post/user/${this.user}?page=${this.posts.pagination.current_page + 1
          }`,
          {
            headers: {
              Authorization: `Bearer ${localStorage.getItem("token")}`,
